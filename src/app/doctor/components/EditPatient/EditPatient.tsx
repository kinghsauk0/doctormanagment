...
import { AllergiesType, DropdownObject, PatientInformationType, PatientResType, DiseaseHistoryType, HabitsType, MedicalHistory, Habit} from "@/app/types";
...
type Props={
    ...
    patientData: PatientResType,
    onGetPatientData : () => void 
}
export default function EditPatient({visible, setVisible,patientData,onGetPatientData}:Props) {
    console.log("data coming",patientData)
    const [habitsList, setHabitsList] = useState<HabitsType[]>(
        (patientData.habit ?? []).map((habit: Habit) => ({
            uid: habit.uid,
    ...
    const [patientInformationData,setPatientInformationData] = useState<PatientInformationType>({
        firstName: patientData.fristName,
        lastName: patientData.lastName,
        mobile: patientData.mobile,
        email:  patientData.email,
        dateOfBirth: parseDateOfBirth(patientData.dateOfBirth),
        gender: {name: patientData.gender},
        age: patientData.age,
        weight: patientData.weight
    })
   
    const [allergiesData,setAllergiesData] = useState<AllergiesType>({
        environmentalAllergies: patientData.allergies.environmentalAllergie,
        foodAllergies: patientData.allergies.foodAllergie,
        drugsAllergies: patientData.allergies.drugAllergie,
        othersAllergies: patientData.allergies.othersAllergies!
    })
     
    const [duration,setDuration] = useState<String>('')
    const [errors, setErrors] = useState<Record<string, string>>({});
    const [diseaseHistoryList, setDiseaseHistoryList] = useState<DiseaseHistoryType[]>(
        patientData.medicalHistory.map((history: MedicalHistory) => ({
            uid: history.uid,
            diseaseName: history.diseaseName,
            diseaseDuration: history.duration,
            diseaseRelated: history.related,
          }))
    );
    const {app} = useApp()
    const ganders = [
    ...
    const EditPatientHeader = (
        <div className="flex align-items-center gap-2">
            <div className='flex gap-4 items-center'>
                ...
                <div>
                    <div className='font-semibold text-base text-[#222]'>Edit patient details</div> 
...
const  handelEditSave  = async() => {
    const userData = {
       ...
       try {
        const validUserData = PatientInformationSchema.parse(userData)
        const validAllergiesData = AllergiesSchema.parse(userAllergies)
        const formData = new FormData();
        formData.append('uid',validUserData.uid)
        formData.append('firstName',validUserData.firstName)
        formData.append('lastName',validUserData.lastName)
        formData.append('age',validUserData.age)
        formData.append('email',validUserData.email)
        formData.append('gender',validUserData.gender)
        formData.append('mobile',validUserData.mobile)
        formData.append('weight',validUserData.weight)
        ...
        formData.append('dateOfBirth',validUserData.dateOfBirth)
        ...
        const { message, status } = await axiosService({
            method: "PATCH",
            url: '/api/doctor/doctor-dashboard/edit-patient',
            body: formData,
          });
  ...
  return (
    <>
        <Sidebar header={EditPatientHeader} visible={visible} position="right" onHide={() => setVisible(false)} className="w-full md:w-[500px]">
            <div className='flex flex-col gap-4 h-full'>
                ...
                <div className="mt-auto pb-3">
                    <Button className="bg-[#0D52AF] text-[#fff] text-sm border-[#0D52AF] w-full" label="Save" onClick={handelEditSave}/>
                </div>
            </div>
        </Sidebar>
                       ...
                       <AccordionTab  key={index} header={his.name}>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                onChange={(e) => setDuration(String(e.value))}
